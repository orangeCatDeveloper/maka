...
export function createDesktopRuntimeHostSshTerminal(input: {
  ...
  return {
    openSshTunnel: async (tunnelInput) => {
      ...
      if (terminal) completePresentation(terminal, true);
      return tunnel;
    },
...
function runtimeHostPackageRemoteCommand(
  ...
  const command = setupPackage.removeAfterSetup
    ? `cd "$HOME" || exit 1; maka_command_exit=0; ${invocation} || maka_command_exit=$?; rm -f -- ${quotePosix(setupPackage.removeAfterSetup)}; exit "$maka_command_exit"`
    : `exec ${invocation}`;
